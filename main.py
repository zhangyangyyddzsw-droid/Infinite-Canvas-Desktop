...
DESKTOP_MODE = env_flag("INFINITE_CANVAS_DESKTOP")
CORS_ALLOW_ORIGINS = [
    origin.strip()
    for origin in os.getenv("INFINITE_CANVAS_CORS_ORIGINS", "*").split(",")
    if origin.strip()
] or ["*"]
...
APP_DATA_DIR = os.path.abspath(os.getenv("INFINITE_CANVAS_DATA_ROOT", BASE_DIR))
WORKFLOW_DIR = os.path.join(APP_DATA_DIR, "workflows")
...
API_ENV_FILE = os.path.join(APP_DATA_DIR, "API", ".env")
...
GLOBAL_CONFIG_FILE = os.path.join(APP_DATA_DIR, "global_config.json")
...
def sync_static_html_versions():
    # PyInstaller resources are read-only application files. Desktop responses
    # already add the current version dynamically, so mutating HTML is neither
    # necessary nor safe in installed builds.
    if DESKTOP_MODE:
        return
    version = current_app_version()
    if not version:
...
def check_update():
    ...
    if DESKTOP_MODE:
        ...
        return {
            "current": current,
            "github": {**disabled, "source": "github"},
            "modelscope": {**disabled, "source": "modelscope"},
            "latest": {},
...
def update_from_github(req: UpdateRequest = UpdateRequest()):
    if DESKTOP_MODE:
        raise HTTPException(status_code=409, detail="桌面版请通过安装包更新，已禁用原地覆盖更新")
...
def rollback_update(req: RollbackRequest):
    if DESKTOP_MODE:
        raise HTTPException(status_code=409, detail="桌面版不支持原地回滚，请重新安装上一版本")
    if not req.name:
...
def gpt_image_2_skill_auth_file():
    ...
    if configured:
        return configured
    project_auth = os.path.join(APP_DATA_DIR, "API", "openai-gpt-account-auth.json")
